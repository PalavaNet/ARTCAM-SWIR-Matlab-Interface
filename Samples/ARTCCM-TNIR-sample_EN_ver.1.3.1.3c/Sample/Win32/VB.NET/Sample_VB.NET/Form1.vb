...
#End Region
    ...
    Dim m_LockFlg As Boolean = False
    Dim PreviewMode As Integer = 0
    
    ' Select SATA camera type when use Sata.dll
    Dim m_CameraType As ARTCAM_CAMERATYPE_SATA = ARTCAM_CAMERATYPE_SATA.ARTCAM_CAMERATYPE_SATA_300MI
    ' ARTCAM_CAMERATYPE_SATA.ARTCAM_CAMERATYPE_SATA_LVDS
    ' ARTCAM_CAMERATYPE_SATA.ARTCAM_CAMERATYPE_SATA_300MI
    ' ARTCAM_CAMERATYPE_SATA.ARTCAM_CAMERATYPE_SATA_500MI
    ' ARTCAM_CAMERATYPE_SATA.ARTCAM_CAMERATYPE_SATA_MV413
    ' ARTCAM_CAMERATYPE_SATA.ARTCAM_CAMERATYPE_SATA_800MI
    ' ARTCAM_CAMERATYPE_SATA.ARTCAM_CAMERATYPE_SATA_036MI
    ' ARTCAM_CAMERATYPE_SATA.ARTCAM_CAMERATYPE_SATA_150P
    ...
    Private Sub MenuCallback_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MenuCallback.Click
        ...
        m_ArtCam.CallBackPreview(Me.Handle(), m_BitmapData.Scan0, getSize(), 1)
    ...
    Private Sub MenuSnapshot_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MenuSnapshot.Click
        ...
        ' Create bit-map
        CreateBitmap()
        m_ArtCam.SnapShot(m_BitmapData.Scan0, getSize(), 1)
        UnLockBitmap()
    ...
    Private Sub MenuTrigger_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MenuTrigger.Click
        ...
        ' An automatic display can be performed, if window size is set up even when using CallBackPreview
        m_ArtCam.SetPreviewWindow(Me.Handle, 0, 0, 0, 0)
        m_ArtCam.Trigger(Me.Handle, m_BitmapData.Scan0, getSize(), 1)
    ...
    Private Sub CreateBitmap()
        ...
        Select Case m_ArtCam.GetColorMode()
            Case 8 : m_Bitmap = New Bitmap(getWidth, getHeight, PixelFormat.Format8bppIndexed)
            ...
            Case 16 : m_Bitmap = New Bitmap(getWidth, getHeight, PixelFormat.Format16bppGrayScale)
    ...
    Private Sub LockBitmap()
        ...
        Select Case m_ArtCam.GetColorMode()
            Case 8 : m_BitmapData = m_Bitmap.LockBits(New Rectangle(0, 0, getWidth, getHeight), ImageLockMode.ReadWrite, PixelFormat.Format8bppIndexed)
            Case 16 : m_BitmapData = m_Bitmap.LockBits(New Rectangle(0, 0, getWidth, getHeight), ImageLockMode.ReadWrite, PixelFormat.Format16bppGrayScale)
            Case 24 : m_BitmapData = m_Bitmap.LockBits(New Rectangle(0, 0, getWidth, getHeight), ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb)
            Case 32 : m_BitmapData = m_Bitmap.LockBits(New Rectangle(0, 0, getWidth, getHeight), ImageLockMode.ReadWrite, PixelFormat.Format32bppRgb)
            Case 48 : m_BitmapData = m_Bitmap.LockBits(New Rectangle(0, 0, getWidth, getHeight), ImageLockMode.ReadWrite, PixelFormat.Format48bppRgb)
            Case 64 : m_BitmapData = m_Bitmap.LockBits(New Rectangle(0, 0, getWidth, getHeight), ImageLockMode.ReadWrite, PixelFormat.Format64bppArgb)
        End Select
    ...
    Private Sub DrawImage16Bit()
        ' 
        Dim DrawBmp As Bitmap = New Bitmap(getWidth, getHeight, PixelFormat.Format8bppIndexed)
        ...
        Dim DrawBmpData As BitmapData = DrawBmp.LockBits(New Rectangle(0, 0, getWidth, getHeight), ImageLockMode.ReadWrite, PixelFormat.Format8bppIndexed)

        Dim pSrc As IntPtr = m_BitmapData.Scan0()
        Dim pDest As IntPtr = DrawBmpData.Scan0()

        ' Color conversion
        Dim Size As Integer = getWidth() * getHeight() - 1
        For i = 0 To Size
        ...
        ' Image display
        Dim g As Graphics = MyBase.CreateGraphics()
        g.DrawImage(DrawBmp, New Rectangle(0, 0, getWidth, getHeight))
        g.Dispose()
    End Sub
